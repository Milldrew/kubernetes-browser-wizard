...
const SETUP_CONTROL_PLANE_NODE_WITH_CILIUM =
  'setup-control-plane-node-with-cilium';
...
export type RecipeTitles =
  ...
  | typeof SETUP_CONTROL_PLANE_NODE_WITH_CILIUM;
export type Slide = {
  /**
...
export const RECIPES: Recipe[] = [
  ...
  {
    recipeTitle: SETUP_CONTROL_PLANE_NODE_WITH_CILIUM,
    slides: [
      {
      ...
      {
        command: `curl -fsSL https://pkgs.k8s.io/core:/stable:/v1.34/deb/Release.key | sudo gpg --dearmor -o /etc/apt/keyrings/kubernetes-apt-keyring.gpg && echo 'deb [signed-by=/etc/apt/keyrings/kubernetes-apt-keyring.gpg] https://pkgs.k8s.io/core:/stable:/v1.34/deb/ /' | sudo tee /etc/apt/sources.list.d/kubernetes.list`,
      ...
      },
      {
        ...
        explanation: `Install the Kubernetes components: kubelet (the node agent), kubeadm (the tool to bootstrap the cluster), and kubectl (the command-line tool to interact with the cluster). Mark them to prevent automatic updates.`,
      },
      {
        command: `sudo swapoff -a && sudo vim /etc/fstab`,
        explanation: `Disable swap memory, which is required for Kubernetes to function properly. Comment out /swap.img → #/swap.img`,
      },
      {
...
EOF`,
      ...
      {
        command: `sudo kubeadm init --skip-phases=addon/kube-proxy`,
        explanation: `Initialize the Kubernetes control plane without kube-proxy since Cilium will replace it. Cilium provides both CNI and kube-proxy functionality.`,
      },
      {
      ...
      {
        command: `curl -L --fail --remote-name-all https://github.com/cilium/cilium-cli/releases/latest/download/cilium-linux-amd64.tar.gz`,
        explanation: `Download the Cilium CLI tool, which will be used to install and manage Cilium in the cluster.`,
      },
      {
        command: `sudo tar xzvfC cilium-linux-amd64.tar.gz /usr/local/bin && rm cilium-linux-amd64.tar.gz`,
        explanation: `Extract the Cilium CLI binary to /usr/local/bin and remove the downloaded archive file.`,
      },
      {
        // command: `cilium install --version 1.15.7`,
        command: `cilium install `,
        explanation: `Install Cilium as the CNI plugin. Cilium provides advanced networking, security, and observability features including replacing kube-proxy functionality.`,
      },
      {
        command: `cilium status --wait`,
        explanation: `Wait for Cilium to be fully deployed and check its status. This ensures that the CNI is properly running before proceeding.`,
      },
      {
        command: `sudo shutdown -r now`,
        explanation: `Reboot the system to ensure all changes take effect and the Kubernetes components are properly initialized with Cilium handling networking.`,
      },
    ],
  },
  {
